...
class LatteA2V(ModelMixin, ConfigMixin):
    ...
    def __init__(
            ...
            encoder_hidden_states_extend: bool = False,
    ):
        super().__init__()
        ...
        self.encoder_hidden_states_extend = encoder_hidden_states_extend
        ...
        image_in_channel = 4 * 10 * 10 # TODO (Xuan): hard code here
        image_out_channel = inner_dim
        self.image_projection = CaptionProjection(in_features=image_in_channel, hidden_size=image_out_channel)
    ...
    def forward(
        ...
        cond_image_num = image_hidden_states.shape[2]
        ...
        #   [batch,                  n  1, key_tokens]
        # this helps to broadcast it as a bias over attention scores, which will be in one of the following shapes:
        #   [batch,  heads, query_tokens, key_tokens] (e.g. torch sdp attn)
        #   [batch * heads, query_tokens, key_tokens] (e.g. xformers or classic attn)

        # 0. (Encoder) Attention Masks
        ...
        # NOTE (Xuan): rewrite encoder_attention_mask
        if encoder_attention_mask is not None and encoder_attention_mask.ndim == 3:
            encoder_attention_mask = (1 - encoder_attention_mask.to(hidden_states.dtype)) * -10000.0
            cond_length = encoder_attention_mask.shape[-1]
            wind_length = int(cond_length / frame)
            if self.encoder_hidden_states_extend:
                encoder_attention_mask_list = []
                for i in range(frame):
                    if i == 0:
                        front_pad = torch.zeros(
                            [input_batch_size, 1, int(wind_length/2)]
                        ).to(encoder_attention_mask.device, encoder_attention_mask.dtype)
                        scale = 2 * wind_length - int(wind_length/2)
                        encoder_attention_mask_item = torch.concat([
                            front_pad, encoder_attention_mask[:,:,:scale]
                        ], dim=2)
                        item_len = encoder_attention_mask_item.shape[2]
                    elif i == frame-1:
                        ...
                        scale = 2 * wind_length - int(wind_length/2)
                        encoder_attention_mask_item = torch.concat([
                            encoder_attention_mask[:,:,-scale:], back_pad
                        ], dim=2)
                    else:
                        item_begin = wind_length * i - int(wind_length/2)
                        item_end = item_begin + item_len
                        encoder_attention_mask_item = encoder_attention_mask[:,:,item_begin:item_end]
                    encoder_attention_mask_list.append(encoder_attention_mask_item)
                # __import__('ipdb').set_trace()
                encoder_attention_mask = torch.stack(encoder_attention_mask_list, dim=2)  
            else:
                hop_length = int(cond_length / frame)
                min_length = hop_length * frame
                encoder_attention_mask = encoder_attention_mask[:,:,:min_length]

            encoder_attention_mask = encoder_attention_mask.reshape(
                input_batch_size * frame, 1, -1
            )
            encoder_attention_mask_img = torch.ones(
                input_batch_size * frame, 1, 
                cond_image_num).to(encoder_attention_mask.device)
            encoder_attention_mask = torch.concat([encoder_attention_mask_img,
                                                   encoder_attention_mask], dim=2)
        ...
        if self.caption_projection is not None:
            ...
            cond_length = encoder_hidden_states.shape[-2]
            hs_dim = encoder_hidden_states.shape[-1]

            # NOTE (Xuan): add front and back frames
            # encoder_hidden_states: [B, 1, T, D]
            if self.encoder_hidden_states_extend:
                wind_length = int(cond_length / frame)
                encoder_hidden_states_list = []
                for i in range(frame):
                    if i == 0:
                        front_pad = torch.ones(
                            [input_batch_size, 1, int(wind_length/2), hs_dim]
                        ).to(encoder_hidden_states.device, encoder_hidden_states.dtype)
                        scale = 2 * wind_length - int(wind_length/2)
                        encoder_hidden_states_item = torch.concat([
                            front_pad, encoder_hidden_states[:,:,:scale,:]
                        ], dim=2)
                        item_len = encoder_hidden_states_item.shape[2]  
                    elif i == frame-1:
                        back_pad = torch.zeros(
                            [input_batch_size, 1, int(wind_length/2), hs_dim]
                        ).to(encoder_hidden_states.device, encoder_hidden_states.dtype)
                        scale = 2 * wind_length - int(wind_length/2)
                        encoder_hidden_states_item = torch.concat([
                            encoder_hidden_states[:,:,-scale:,:], back_pad
                        ], dim=2)
                    else:
                        item_begin = wind_length * i - int(wind_length/2)
                        item_end = item_begin + item_len
                        encoder_hidden_states_item = encoder_hidden_states[:,:,item_begin:item_end,:]
                    encoder_hidden_states_list.append(encoder_hidden_states_item)
                encoder_hidden_states = torch.stack(encoder_hidden_states_list, dim=2).squeeze(1)
            else:
                wind_length = round(cond_length / frame)
                min_length = wind_length * frame
                encoder_hidden_states = encoder_hidden_states[:,:,:min_length,:]
                encoder_hidden_states = encoder_hidden_states.view(
                    input_batch_size, 1, frame, wind_length, hs_dim
                ).squeeze(1)
            
            encoder_hidden_states_spatial = encoder_hidden_states.reshape(
                input_batch_size * frame, -1, hs_dim
            )
            image_hidden_states = image_hidden_states.view(input_batch_size, cond_image_num, -1)
            encoder_hidden_states_img = self.image_projection(image_hidden_states)
            encoder_hidden_states_img = repeat(encoder_hidden_states_img, 'b n d -> (b f) n d', f=frame).contiguous()
            encoder_hidden_states_spatial = torch.concat([
                encoder_hidden_states_img,
                encoder_hidden_states_spatial], dim=1)
            encoder_hidden_states_spatial = encoder_hidden_states_spatial.to(torch.float16) # NOTE(Xuan): bfloat16
        ...
        for i, (spatial_block, temp_block) in enumerate(zip(self.transformer_blocks, self.temporal_transformer_blocks)):
            ...
            if self.training and self.gradient_checkpointing:
                ...
                if enable_temporal_attentions:
                    ...
                    if use_image_num != 0:  # image-video joint training
...
# depth = num_layers * 2
def LatteA2V_XL_122(**kwargs):
    return LatteA2V(num_layers=28, attention_head_dim=72, num_attention_heads=16, patch_size_t=1, patch_size=2,
                    norm_type="ada_norm_single", caption_channels=768, cross_attention_dim=1152, **kwargs)
def LatteA2V_D64_XL_122(**kwargs):
    return LatteA2V(num_layers=28, attention_head_dim=64, num_attention_heads=18, patch_size_t=1, patch_size=2,
                    norm_type="ada_norm_single", caption_channels=4096, cross_attention_dim=1152, **kwargs)

Latte_models_A2V = {
    "LatteA2V-XL/122": LatteA2V_XL_122,
    "LatteA2V-D64-XL/122": LatteA2V_D64_XL_122,
}
