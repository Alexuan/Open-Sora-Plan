...
def main(args):
    ...
    if args.ae_ckpt_dir:
        ...
        ae.load_state_dict(ae_ckpt_sd_update)
    ...
    audio_enc = get_audio_warpper(args.audio_encoder_name)(args, **kwargs).eval()
    ...
    model = Diffusion_models[args.model](
        ...
        encoder_hidden_states_extend=args.encoder_hidden_states_extend,
    )
    ...
    if args.pretrained:
        ...
        model_state_dict = model.state_dicut()
    ...
    # Move unet, vae and audio_encoder to device and cast to weight_dtype
    # The VAE is in float32 to avoid NaN losses.
    # ae.to(accelerator.device, dtype=torch.float32)
    ae.to(accelerator.device, dtype=weight_dtype)
    # ae.to(accelerator.device)
    audio_enc.to(accelerator.device)

    # Create EMA for the unet.
    if args.use_ema:
        ema_model = deepcopy(model)
        ema_model = EMAModel(ema_model.parameters(), model_cls=LatteA2V, model_config=ema_model.config)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "model_ema"), LatteA2V)
            ...
            for i in range(len(models)):
                ...
                load_model = LatteA2V.from_pretrained(input_dir, subfolder="model")
    ...
    # Setup data:
    def sanity_check_video_audio(video, audio):
        os.makedirs("debug", exist_ok=True)
        from examples.rec_video_vae import array_to_video, custom_to_video
        video = custom_to_video(video, fps=17, output_file="debug/output_video.mp4")
        
        audio = audio.detach().cpu().numpy()
        from scipy.io import wavfile
        wavfile.write("debug/outout_audio.wav", rate=20000, data=audio.astype(np.int16))


    train_dataset = getdataset(args)
    # note: sanity check of input
    # rt_data = train_dataset.__getitem__(0)
    # sanity_check_video_audio(rt_data['video_data']['video'], rt_data['video_data']['audio'])
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        shuffle=True,
        collate_fn=CollateAI2V(args),
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        train_loss = 0.0
        for step, (x, attn_mask, cond_img, cond_aud, input_ids, cond_mask, file_name) in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                # Sample noise that we'll add to the latents
                if not args.multi_scale:
                    assert torch.all(attn_mask)
                x = x.to(accelerator.device, dtype=weight_dtype)  # B C T H W
                attn_mask = attn_mask.to(accelerator.device)  # B 1 L

                cond_aud = cond_aud.to(accelerator.device, dtype=torch.float32)
                cond_img = cond_img.to(accelerator.device, dtype=torch.float32)
                ...
                with torch.no_grad():
                    ...
                    cond_audio, cond_audio_mask = audio_enc(cond_aud)
                    cond_audio = cond_audio.to(dtype=weight_dtype).unsqueeze(dim=1)
                    cond_audio_mask = cond_audio_mask.to(dtype=weight_dtype).unsqueeze(dim=1).to(accelerator.device)
                    
                    if args.same_modality:
                        cond_img = x[:,:,0:1,:,:].to(accelerator.device, dtype=torch.float32)

                    # Map input images to latent space + normalize latents
                    videos = ae.encode(x)  # B C T H W
                    ...
                    cond_images = rearrange(cond_img, 'b c t h w -> (b t) c 1 h w')
                    cond_images = cond_images.to(accelerator.device, dtype=weight_dtype)
                    cond_images = ae.encode(cond_images)

                    cond_images = rearrange(cond_images, '(b t) c 1 h w -> b c t h w', t=args.cond_image_num)


                # print('(x.shape, attn_mask.shape, cond.shape, cond_mask.shape', x.shape, attn_mask.shape, cond.shape, cond_mask.shape)
                model_kwargs = dict(encoder_hidden_states=cond_audio, 
                                    attention_mask=attn_mask,
                                    encoder_attention_mask=cond_audio_mask,
                                    image_hidden_states=cond_images,
                                    use_image_num=args.use_image_num)
                t = torch.randint(0, diffusion.num_timesteps, (x.shape[0],), device=accelerator.device)
                # __import__('ipdb').set_trace()
                loss_dict = diffusion.training_losses(model, videos, t, model_kwargs)
